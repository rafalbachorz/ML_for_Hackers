...
heights.weights$Gender <- gsub("Male", "Mê¿czyzna", heights.weights$Gender)
...
# Why only single line?
is.factor(heights.weights$Gender)

# Get info about the nature of the data (factor/numeric/character) 
str(heights.weights)
...
# Supplement a bit
heights.weights <- rbind(heights.weights, c("Mê¿czyzna", NA, 88.0))
heights.weights <- rbind(heights.weights, c("Mê¿czyzna", 167.0, NA))
...
# Code from polish book
data.file <- file.path('data', '01_heights_weights_genders.csv')
heights.weights <- read.csv(data.file, header = TRUE, sep = ',')
# Przelicz jednostki i zamieñ symbole p³ci
heights.weights$Height <- heights.weights$Height * 2.54
heights.weights$Weight <- heights.weights$Weight * 0.454
heights.weights$Gender <- gsub("Male", "Mê¿czyzna", heights.weights$Gender)
heights.weights$Gender <- gsub("Female", "Kobieta", heights.weights$Gender)
ggplot(heights.weights, aes(x = Height)) +
  geom_histogram(binwidth = 2) + labs(x = "Wzrost", y = "liczba")


ggplot(heights.weights, aes(x = Height)) + geom_density() + labs(x = "Wzrost", y = "gêstoœæ")

ggplot(heights.weights, aes(x = Height, fill = Gender)) + geom_density() + labs(x = "Wzrost", y = "gêstoœæ", fill = "P³eæ")

ggplot(heights.weights, aes(x = Weight, fill = Gender)) + geom_density() + labs(x = "Waga", y = "gêstoœæ", fill = "P³eæ")

ggplot(heights.weights, aes(x = Height, fill = Gender)) + geom_density() + labs(x = "Wzrost", y = "gêstoœæ", fill = "P³eæ") + facet_grid(Gender ~ .)
...
ggplot(heights.weights, aes(x = Height, y = Weight, color = Gender)) + geom_point() + labs(x = "Wzrost", y = "Waga", color = "P³eæ")

heights.weights <- transform(heights.weights, Male = ifelse(Gender == 'Mê¿czyzna', 1, 0))

logit.model <- glm(Male ~ Height + Weight, data = heights.weights, family = binomial(link = 'logit'))
ggplot(heights.weights, aes(x = Weight, y = Height, color = Gender)) + geom_point() + stat_abline(intercept = -coef(logit.model)[1] / coef(logit.model)[2],
